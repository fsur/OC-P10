...
class BookingDialog(CancelAndHelpDialog):
    ...
    def __init__(
        ...
        self.add_dialog(
            DateResolverDialog(DateResolverDialog.__name__+"_start", self.telemetry_client,isStart=True)
        )
        self.add_dialog(
            DateResolverDialog(DateResolverDialog.__name__+"_end", self.telemetry_client,isStart=False)
        )
    ...
    async def start_travel_date_step(
        ...
        if not booking_details.start_travel_date or self.is_ambiguous(
            ...
            return await step_context.begin_dialog(
                DateResolverDialog.__name__+"_start", booking_details.start_travel_date
    ...
    async def end_travel_date_step(
        ...
        if not booking_details.end_travel_date or self.is_ambiguous(
            ...
            return await step_context.begin_dialog(
                DateResolverDialog.__name__+"_end", booking_details.end_travel_date
            )  # pylint: disable=line-too-long
    ...
    async def budget_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        if booking_details.budget is None:
            return await step_context.prompt(
                ...
                PromptOptions(
                    prompt=MessageFactory.text("What budget do you have for your trip?")
                ),
            )  # pylint: disable=line-too-long,bad-continuation
    ...
    async def confirm_step(
        ...
        # Capture the results of the previous step
        origin = booking_details.origin.lower().title()
        destination = booking_details.destination.lower().title()
        booking_details.budget = step_context.result
        msg = (
            f"Please confirm, I have you traveling "
            f"from: { origin } "
            f"to: { destination } "
            f"on: { booking_details.start_travel_date} "
            f"and return on: { booking_details.end_travel_date} "
